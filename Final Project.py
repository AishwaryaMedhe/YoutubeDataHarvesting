...
api = "AIzaSyDD4wk8FCZZGVtHDcUKioBh1MsrX9FpAl4"
c_id = "UCC6gbNO098rkQKCU0ZaLITQ"
# Mongodb uploading
client = pymongo.MongoClient(
    "mongodb+srv://Aishwarya:Aishwarya@cluster0.qigwtrl.mongodb.net/?retryWrites=true&w=majority")
...
mycon = pymysql.connect(host="127.0.0.1", user="root", password="Dreamer_1")
...
def channel_details(channel_id):
    ...
    # pprint(curr_chan)
    video_id = []
    playlist_response = (youtube.playlistItems().list(part='snippet', playlistId=(
    Channel_data['Channel_name']["Playlist_Id"]))).execute()
    ...
    for i in range(1, len(result) - 1):
        ...
        for j in range(1, len(result[f"video_id_{i}"]['comments'])):
            comment_data_sql = {"Comment_Id": result[f"video_id_{i}"]['comments'][f"Comment_Id_{j}"]['Comment_Id'],
                                ...
                                "Comment_Author": result[f"video_id_{i}"]['comments'][f"Comment_Id_{j}"][
                                    'Comment_Author'],
                                "Comment_PublishedAt": result[f"video_id_{i}"]['comments'][f"Comment_Id_{j}"][
                                    'Comment_PublishedAt']}
...
def table(df):
    ...
    engine = create_engine('mysql+pymysql://root:Dreamer_1@localhost/youtube', echo=False)
...
if nav == "HOME":
    ...
    if st.button("Run") and len(st.session_state.ch_id) == 24:
        st.session_state.results = channel_details(st.session_state.ch_id)  # initialize fetching the data
        st.balloons()
        ...
        mycon = mysql.connector.connect(host="127.0.0.1", user="root", password="Dreamer_1", database='youtube')
        mycursor = mycon.cursor()
